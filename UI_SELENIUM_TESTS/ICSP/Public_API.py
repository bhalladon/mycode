...
class api(unittest.TestCase):
    ...
    def test_a_apidocs(self):
        ...
        try:
            element = WebDriverWait(b,10).until(EC.presence_of_element_located,((By.LINK_TEXT,"ICS Protection API Docs"))
            )
        except:
            print "Checkpoint 4: [FAILED] API Docs link not found."
            print self.fail("Case Status: FAILED")
            return 0
        print "Checkpoint 4: [PASSED] API Docs link found."
        time.sleep(2)
        api_link = b.find_element_by_xpath('//*[@id="navbar_buttons"]/ul/li[6]/ul/li[1]/a').get_attribute("target")
        
        if (api_link == "_blank"):
            print "Checkpoint 5: [PASSED] API Docs opens up in new page."
        else:
            print "Checkpoint 5: [FAILED] API Docs opens up in same window."
            print self.fail("Case Status: FAILED.")
            return 0
