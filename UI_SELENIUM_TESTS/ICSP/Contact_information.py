...
import unittest,time # sys, os, time
...
class helpandtroubleshooting(unittest.TestCase):
    ...
    def test_a_helplink(self):
        ...
        help_link = b.find_element_by_link_text("Contact information").get_attribute("target")
        if (help_link == "_blank"):
            print "Checkpoint 4: [PASSED] Contact Information opens up in new page."
        else:
            print "Checkpoint 4: [FAILED] Contact Information opens up in same window."
            print self.fail("Case Status: FAILED.")
            return 0
